$(document).ready(function() {
  ...
  function crearFilas(datos) {
    ...
    for (let i = 0; i < datos.length; i++) {
      ...
      fila.html('<td>' + datos[i].id + '</td>' +
                ...
                '<td>' + datos[i].fecha_inseminacion + '</td>' +
                '<td>' + datos[i].fecha_aproxParto + '</td>' +
                '<td>' + datos[i].fecha_ultimoParto + '</td>' +
                '<td>' + datos[i].fecha_revision + '</td>' +
                '<td>' + datos[i].categoria + '</td>' +
                '<td><button class="delete-button" data-id="' + datos[i].id + '">Eliminar</button></td>'+
                '<td><a class="btn-actualizar" href="/Empleado/ActualizarGestacion.html?id=' + datos[i].id + '">Actualizar</a></td>');
      tbody.append(fila);
    }
  }
  ...
  function obtenerDatos() {
    $.ajax({
      url: 'https://vaquijpa2-production.up.railway.app/listarGestacion',
      type: "GET",
      dataType: "JSON",
  ...
  obtenerDatos();
    ...
    function eliminarFila() {
      ...
      if (confirm("¿Estás seguro de que deseas eliminar el bovino?")) {
        $.ajax({
          url: 'https://vaquijpa2-production.up.railway.app/eliminarGestacion/' + id,
          type: "DELETE",
          dataType: "JSON",
          success: function(respuesta) {
            console.log(respuesta);
