$(document).ready(function() {
  ...
  function crearFilas(datos) {
    ...
    for (let i = 0; i < datos.length; i++) {
      ...
      fila.html('<td>' + datos[i].id + '</td>' +
                ...
                '<td><a class="btn-actualizar" href="/Empleado/ActualizarToros.html?id=' + datos[i].id + '">Actualizar</a></td>');
      tbody.append(fila);
    }
  }
  ...
  function obtenerDatos() {
    $.ajax({
      url: 'https://vaquijpa2-production.up.railway.app/listarToros',
      type: "GET",
      dataType: "JSON",
  ...
  obtenerDatos();
    ...
    function eliminarFila() {
      ...
      if (confirm("¿Estás seguro de que deseas eliminar el bovino?")) {
        $.ajax({
          url: 'https://vaquijpa2-production.up.railway.app/eliminarBovinoGeneral/' + id,
          type: "DELETE",
          dataType: "JSON",
          success: function(respuesta) {
            console.log(respuesta);
