$(document).ready(function() {
  ...
  function crearFilas(datos) {
    ...
    for (let i = 0; i < datos.length; i++) {
      ...
      fila.html('<td>' + datos[i].id + '</td>' +
                ...
                '<td><a class="btn-actualizar" href="/Empleado/ActualizarTerneros.html?id=' + datos[i].id + '">Actualizar</a></td>');
      tbody.append(fila);
    }
  }
  ...
  function obtenerDatos() {
    $.ajax({
      url: "http://vaquijpa2-production.up.railway.app/listarTerneros",
      type: "GET",
      dataType: "JSON",
  ...
  obtenerDatos();
    ...
    function eliminarFila() {
      ...
      if (confirm("¿Estás seguro de que deseas eliminar el bovino?")) {
        $.ajax({
          url: "http://vaquijpa2-production.up.railway.app/eliminarTernero/" + id,
          type: "DELETE",
          dataType: "JSON",
          success: function(respuesta) {
            console.log(respuesta);
